...
function run_to_door.load(x, w, h)
    ...
    player = sodapop.newAnimatedSprite(run_to_door.w / 4 * 3, run_to_door.h / 2)
    ...
    enemy = sodapop.newSprite(love.graphics.newImage('assets/ufo_enemy.png'), run_to_door.w / 8 * 7 , run_to_door.h / 2)
...
function run_to_door.update(dt)
    player:update(dt)
end
